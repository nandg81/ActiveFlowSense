...
Controller for ActiveFlowSense with zero utilization displayed
'''
...
'''
mac_to_port = {}
#The prv_utilization data structure is used to keep track of the previous time at which the utilization for a link was calculated
prv_utilization = {}
timeout_value = 0
timer_value = 0
hard_or_soft = 2
...
#function that gets executed recurrently and does active probing if needed
def _timer_func ():
	global current_time,active_list,timer_value,prv_utilization,mac_to_port
	timestamp=get_the_time()
	#if there is an active flow in active_list and the previous utilization for the corresponding link was calculated more than timer_value seconds ago, do active probing for that port
	for flow in active_list:
		if flow['active']==1 and current_time-prv_utilization[flow['dpid']][mac_to_port[flow['dpid']][flow['dst']]]['time']>=timer_value:
			for connection in core.openflow.connections:
				if(connection.dpid==flow['dpid']):
					req=of.ofp_flow_stats_request()
					#send active probe for the output port corresponding to the flow
					req.out_port=mac_to_port[flow['dpid']][flow['dst']]
					connection.send(of.ofp_stats_request(body=req))
	
	#if there is no active flow for a particular port and previous utilization was calculated more than timer_value seconds ago, display zero utilization
	for dpid in mac_to_port:
		for port in prv_utilization[dpid]:
			flag=0
			for flow in active_list:
				if flow['active']==1 and flow['dpid']==dpid and mac_to_port[flow['dpid']][flow['dst']]==port:
					flag=1
			#if there was an active flow, flag would be 1
			if flag == 0 and current_time-prv_utilization[dpid][port]['time']>=timer_value:
				file1 = open("output.txt","a")
				str1="Time: "+str(timestamp)+ " Seconds: "+str(current_time)+" Utilization on link from switch "+str(dpid)+" through port "+ str(port) + " = 0 kbps"
				print(str1)
				file1.write(str1)
				file1.write("\n")
				file1.close()
				save_utilization_data(dpid,port,0)
...
#function to store the previous time at which utilization for a particular link was calculated
def save_utilization_data (dpid,port,utilization):
	...
	prv_utilization[dpid][port]['utilization']=utilization
...
def _handle_FlowRemoved (event):
	...
	if matching_flow['end_time']-matching_flow['start_time']>0:
		...
		if(checkpoint['active']==0):
			save_utilization_data(event.connection.dpid,mac_to_port[event.connection.dpid][event.ofp.match.dl_dst],checkpoint['utilization'])
		file1 = open("output.txt","a")
		...
		for cp in utilization_table:
			if cp['dpid']==event.connection.dpid and cp['dst']==event.ofp.match.dl_dst and cp['active']>0 and cp['time']>matching_flow['start_time'] and cp['time']<checkpoint['time']:
				...
				str1="Previous Checkpoint: "+str(cp['time'])+" Utilization on link from switch "+str(event.connection.dpid)+" through port "+ str(mac_to_port[event.connection.dpid][event.ofp.match.dl_dst]) + " = "+ str(cp['utilization']) + " kbps Flows remaining: "+ str(cp['active'])
				print(str1)
				file1.write(str1)
				file1.write("\n")
				if(cp['active']==0):
...
def _handle_ConnectionUp(event):
	global mac_to_port, prv_utilization, start_time
	if start_time==0:
		start_time=time.mktime(time.localtime())
	#initialise the outer dictionary of mac_to_port with key values as switch dpids as the connection with switch gets established and with empty values
	mac_to_port[event.connection.dpid]={}
	#initialise the outer dictionary of prv_utilization with key values as switch dpids as the connection with switch gets established and with empty values
	...
	for swprt in event.connection.features.ports:
		if swprt.port_no != 65534:
			print(swprt.name)
			#initialize prv_utilization for every port as 0
...
def _handle_PacketIn(event):
	global mac_to_port,hard_or_soft,timeout_value
...
'''
handle flowStatsReceived : 
Fired when the controller receives a flow stats packet. It consists of a list, with each element corresponding to a particular flow
'''
def _handle_flowstats_received (event):
	global current_time,mac_to_port,prv_utilization,timer_value
	timestamp=get_the_time()
	dpid = event.connection.dpid
	utilization=0
	port=0
	...
	if(current_time-prv_utilization[dpid][port]['time']>=timer_value):
		file1 = open("output.txt","a")
		str1="Active probing - Time: "+str(timestamp)+ " Seconds: "+str(current_time)+" Utilization on link from switch "+str(dpid)+" through port "+ str(port) + " = "+ str(utilization) + " kbps"
		print(str1)
		file1.write(str1)
		file1.write("\n")
		file1.close()
...
'''			
def launch():
	global hard_or_soft,timeout_value,timer_value
	...
	Timer(1, _timer_func, recurring=True) #to execute the timer function recurrently
